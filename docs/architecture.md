...
Plans and approvals live in `plugload-operations`; immutable audit events live in `plugload-audit-events`. IDs are unguessable UUIDs, plans expire, approvals are bound to one plan, and apply compares a stable SHA-256 content hash with the preview baseline.
